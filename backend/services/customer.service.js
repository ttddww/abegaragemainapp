...
async function editCustomer(updatedCustomerData) {
  ...
  if (
    ...
    await conn.query(customerQuery, queryParams);
  }

  if (updatedCustomerData.customer_email || updatedCustomerData.customer_phone_number) {
    const customerIdentifierQuery = `
        UPDATE customer_identifier
        SET 
          ${updatedCustomerData.customer_email ? "customer_email = ?," : ""}
          ${updatedCustomerData.customer_phone_number ? "customer_phone_number = ?" : ""}
        WHERE
          customer_id = ?
      `;
    const queryParams = [
      updatedCustomerData.customer_email,
      updatedCustomerData.customer_phone_number,
      updatedCustomerData.customer_id,
    ].filter((param) => param !== undefined);

    await conn.query(customerIdentifierQuery, queryParams);
  }
